...
function RatingInput({ value, onChange }) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        ...
        return (
          <button
            ...
            style={{
              ...
              border: `2px solid ${active ? HUGVIT_LIGHT : hovered ? HUGVIT_MED : GRAY_200}`,
              background: active ? HUGVIT_BLUE : hovered ? HUGVIT_BG : WHITE,
              color: active ? WHITE : hovered ? HUGVIT_BLUE : GRAY_800,
      ...
      <span style={{ fontSize: 13, color: GRAY_400, marginLeft: 8, fontStyle: "italic" }}>
        {hover ? ratingLabels[hover - 1] : value ? ratingLabels[value - 1] : ""}
      </span>
    </div>
  );
}
...
function NPSInput({ value, onChange }) {
  ...
  return (
    <div>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: `2px solid ${active ? c : hovered ? c + "80" : GRAY_200}`,
                background: active ? c : hovered ? c + "12" : WHITE,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: GRAY_400 }}>
        <span>Mjög ólíklegt</span>
  ...
  return (
    <textarea
      ...
      style={{
        ...
        border: `2px solid ${GRAY_200}`, fontSize: 15,
...
function ThankYou({ answers }) {
  const ratings = Object.entries(answers)
    .filter(([k, v]) => typeof v === "number" && v >= 1 && v <= 5 && k !== "q19")
    .map(([, v]) => v);
  ...
  return (
    <div style={{ textAlign: "center", padding: "60px 20px" }}>
      <h2 style={{ fontSize: 28, color: HUGVIT_BLUE, fontWeight: 800, marginBottom: 12 }}>
        Takk fyrir þátttökuna
      </h2>
      <p style={{ fontSize: 16, color: GRAY_600, maxWidth: 460, margin: "0 auto 36px", lineHeight: 1.7 }}>
        Svör þín hjálpa okkur að bæta þjónustu okkar. Við munum fara yfir athugasemdir
        þínar og nota þær til að bæta framtíðar vinnupakka.
      </p>
      <div style={{ display: "flex", gap: 20, justifyContent: "center", flexWrap: "wrap", marginBottom: 40 }}>
        ...
        {nps !== undefined && (
          <div style={{
            background: nps >= 9 ? SUCCESS : nps >= 7 ? WARN : DANGER,
            color: WHITE, borderRadius: 14, padding: "20px 36px", minWidth: 140,
          }}>
...
export default function Page() {
  ...
  const answeredCount = allQuestions.filter((q) => {
    if (q.condition) {
      const condVal = answers[q.condition.id];
      if (!q.condition.values.includes(condVal)) return true;
    }
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            ...
            <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", letterSpacing: 0.5 }}>Ánægjukönnun</span>
          </div>
          ...
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", margin: "0 0 18px" }}>
            Vinnupakki — Endurgjöf viðskiptavinar
          </p>
          {/* Progress */}
          ...
          return (
            <button
              ...
              style={{
                ...
                background: active ? HUGVIT_BLUE : done ? SUCCESS + "12" : GRAY_100,
                color: active ? WHITE : done ? SUCCESS : GRAY_600,
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 24px 120px" }}>
        <div style={{
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 800, color: HUGVIT_BLUE, margin: "6px 0 0" }}>
              {section.title}
            </h2>
          </div>
          ...
          {section.questions.map((q, qi) => {
            if (q.condition) {
              const condVal = answers[q.condition.id];
              if (!q.condition.values.includes(condVal)) return null;
            }
            return (
              <div key={q.id} style={{
                ...
                <label style={{ display: "block", fontSize: 15, fontWeight: 600, color: GRAY_800, marginBottom: 12, lineHeight: 1.5 }}>
                  {q.text}
                </label>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, gap: 12 }}>
          <button
            onClick={() => currentSection > 0 && goTo(currentSection - 1)}
            disabled={currentSection === 0}
            style={{
              padding: "12px 24px", borderRadius: 10,
              border: `2px solid ${GRAY_200}`, background: WHITE,
          ...
          {isLast ? (
            <button
              onClick={() => {
                console.log("CSAT_SUBMISSION", JSON.stringify(answers, null, 2));
                setSubmitted(true);
              }}
              style={{
