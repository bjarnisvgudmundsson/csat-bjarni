...
const expectations = [
  ...
  { title: "Þetta er ekki próf", desc: "Við erum ekki að meta ykkar vinnu. Við erum að leita að tækifærum til að gera hlutina einfaldari." },
];
...
function ChoiceInput({ options, value, onChange }) {
  return <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{options.map((opt) => (<button key={opt} onClick={() => onChange(opt)} style={{ padding: "9px 18px", borderRadius: 10, border: `2px solid ${value === opt ? HUGVIT_LIGHT : GRAY_200}`, background: value === opt ? HUGVIT_BLUE : WHITE, color: value === opt ? WHITE : GRAY_800, fontSize: 14, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}>{opt}</button>))}</div>;
}
...
export default function ParticipantOnboarding() {
  ...
  const NavDots = () => (
    <div style={{ display: "flex", justifyContent: "center", gap: 8, margin: "20px 0" }}>
      {steps.map((s, i) => (<div key={s} style={{ width: step === s ? 24 : 8, height: 8, borderRadius: 4, background: step === s ? HUGVIT_BLUE : steps.indexOf(step) > i ? SUCCESS : GRAY_200, transition: "all 0.3s ease" }} />))}
    </div>
  );
      ...
      <div style={{ minHeight: "100vh", background: HUGVIT_BG }}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 24px 80px" }}>
          ...
          <div style={{ background: WHITE, borderRadius: 16, padding: "32px 24px", boxShadow: "0 1px 12px rgba(26,60,97,0.05)" }}>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: HUGVIT_BLUE, margin: "0 0 8px" }}>Velkomin/n</h2>
            <p style={{ fontSize: 15, color: GRAY_600, lineHeight: 1.7, margin: "0 0 28px" }}>Þú hefur verið boðið til að taka þátt í vinnustofu um gervigreind í málastjórnun. Vinnustofan er hluti af vinnupakka sem stofnun þín keypti af Hugvit og miðar að því að kanna hvernig gervigreind getur aukið skilvirkni í ykkar daglegri vinnu.</p>

            <h3 style={{ fontSize: 16, fontWeight: 700, color: HUGVIT_BLUE, margin: "0 0 16px" }}>Hvernig vinnupakkinn virkar</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
              {phases.map((p) => (
                <div key={p.num} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                  <div style={{ minWidth: 36, height: 36, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", background: HUGVIT_BLUE, color: WHITE, fontSize: 13, fontWeight: 800 }}>{p.num}</div>
                  <div style={{ flex: 1 }}>
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, color: HUGVIT_BLUE, margin: "0 0 16px" }}>Hvað þú getur vænst</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {expectations.map((e, i) => (
                <div key={i} style={{ background: HUGVIT_BG, borderRadius: 10, padding: "14px 16px", border: `1px solid ${GRAY_200}` }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
            <button onClick={() => setStep("prepare")} style={{ padding: "12px 28px", borderRadius: 10, border: "none", background: HUGVIT_BLUE, color: WHITE, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Áfram</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ minHeight: "100vh", background: HUGVIT_BG }}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 24px 80px" }}>
          ...
          <div style={{ background: WHITE, borderRadius: 16, padding: "32px 24px", boxShadow: "0 1px 12px rgba(26,60,97,0.05)", marginBottom: 16 }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: HUGVIT_BLUE, margin: "0 0 8px" }}>Dæmigerð dagskrá</h2>
            <p style={{ fontSize: 14, color: GRAY_600, lineHeight: 1.5, margin: "0 0 20px" }}>Dagskráin er sveigjanleg og ráðgjafi lagar hana að ykkar þörfum. Hér er dæmigerð uppbygging:</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {dayAgenda.map((item, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", gap: 16, padding: "10px 0", borderBottom: i < dayAgenda.length - 1 ? `1px solid ${GRAY_200}` : "none", opacity: isBreak ? 0.5 : 1 }}>
          ...
          <div style={{ background: WHITE, borderRadius: 16, padding: "28px 24px", boxShadow: "0 1px 12px rgba(26,60,97,0.05)" }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: HUGVIT_BLUE, margin: "0 0 8px" }}>Hvað er gervigreind í málastjórnun?</h2>
            <p style={{ fontSize: 14, color: GRAY_600, lineHeight: 1.7, margin: "0 0 16px" }}>Gervigreind (e. artificial intelligence) er tækni sem getur lesið, skilið og dregið saman texta á nokkrum sekúndum. Í málastjórnun þýðir þetta að kerfið getur:</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
              {["Lesið innkomið erindi og dregið saman kjarnann á nokkrum sekúndum", "Flokkað mál sjálfkrafa eftir tegund, forgangsstigi eða efni", "Ráðlagt hvaða starfsmanni málið ætti að vera úthlutað", "Spáð fyrir um vinnuálag og flöskuhálsa", "Fundið tengd mál og fordæmi úr eldri málum"].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <div style={{ minWidth: 22, height: 22, borderRadius: 6, background: HUGVIT_BLUE + "10", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: HUGVIT_BLUE, marginTop: 1 }}>{i + 1}</div>
                  <span style={{ fontSize: 14, color: GRAY_800, lineHeight: 1.5 }}>{item}</span>
                </div>
              ))}
            </div>
            <div style={{ background: HUGVIT_BG, borderRadius: 10, padding: "16px 18px", border: `1px solid ${GRAY_200}` }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: HUGVIT_BLUE, margin: "0 0 4px" }}>Mundu</p>
              <p style={{ fontSize: 14, color: GRAY_600, margin: 0, lineHeight: 1.5 }}>Gervigreindin er verkfæri sem styður þig, ekki kemur í staðinn fyrir þig. Á vinnustofunni munum við skoða saman hvar hún getur hjálpað mest í ykkar vinnu.</p>
            </div>
          </div>

          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, gap: 12 }}>
            <button onClick={() => setStep("welcome")} style={{ padding: "12px 24px", borderRadius: 10, border: `2px solid ${GRAY_200}`, background: WHITE, color: GRAY_800, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Til baka</button>
            <button onClick={() => setStep("form")} style={{ padding: "12px 28px", borderRadius: 10, border: "none", background: HUGVIT_BLUE, color: WHITE, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Áfram</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div style={{ minHeight: "100vh", background: HUGVIT_BG }}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 24px 80px" }}>
          <NavDots />
          <div style={{ background: WHITE, borderRadius: 16, padding: "28px 24px", boxShadow: "0 1px 12px rgba(26,60,97,0.05)" }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: HUGVIT_BLUE, margin: "0 0 8px" }}>Stuttar spurningar</h2>
            <p style={{ fontSize: 14, color: GRAY_600, lineHeight: 1.5, margin: "0 0 24px" }}>Þetta tekur 2–3 mínútur. Svörin hjálpa ráðgjafa að sérsníða vinnustofuna að ykkar hóp.</p>
            {questions.map((q, qi) => (
              <div key={q.id} style={{ marginBottom: 22, paddingBottom: 22, borderBottom: qi < questions.length - 1 ? `1px solid ${GRAY_200}` : "none" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, gap: 12 }}>
            <button onClick={() => setStep("prepare")} style={{ padding: "12px 24px", borderRadius: 10, border: `2px solid ${GRAY_200}`, background: WHITE, color: GRAY_800, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Til baka</button>
            <button onClick={() => { console.log("PARTICIPANT_ONBOARDING", JSON.stringify(answers, null, 2)); setStep("done"); }} style={{ padding: "12px 32px", borderRadius: 10, border: "none", background: SUCCESS, color: WHITE, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Senda inn</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: "100vh", background: HUGVIT_BG, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: WHITE, borderRadius: 20, maxWidth: 560, width: "100%", margin: 20, boxShadow: "0 8px 40px rgba(26,60,97,0.08)", textAlign: "center", padding: "60px 28px" }}>
        ...
        <p style={{ fontSize: 16, color: GRAY_600, maxWidth: 420, margin: "0 auto 28px", lineHeight: 1.7 }}>Þú ert tilbúin/n. Ráðgjafi Hugvits mun nota svör þín til að sérsníða vinnustofuna. Þú færð dagskrá og staðfestingu í tölvupósti áður en vinnustofan hefst.</p>
        <div style={{ background: HUGVIT_BG, borderRadius: 12, padding: "18px 22px", maxWidth: 380, margin: "0 auto 32px", textAlign: "left", border: `1px solid ${GRAY_200}` }}>
